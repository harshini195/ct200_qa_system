...
@app.get("/documents/{document_name}/versions")
def list_versions(document_name: str, db: Session = Depends(get_db)):
    doc = db.query(models.Document).filter_by(name=document_name).first()
    if doc is None:
        raise HTTPException(404, "document not found")
    import json as _json
    versions = crud.list_versions(db, doc.id)
    return [
        {
            "version_number": v.version_number,
            "ingested_at": v.ingested_at.isoformat(),
            "parser_warnings": _json.loads(v.parser_warnings or "[]"),
        }
        for v in versions
    ]


@app.get("/documents/{document_name}/versions/{version_number}")
def get_version_detail(document_name: str, version_number: int, db: Session = Depends(get_db)):
    doc = db.query(models.Document).filter_by(name=document_name).first()
    if doc is None:
        raise HTTPException(404, "document not found")
    ver = crud.resolve_version(db, doc.id, version_number)
    if ver is None:
        raise HTTPException(404, "version not found")
    import json as _json
    node_count = db.query(models.NodeRevision).filter_by(version_id=ver.id).count()
    return {
        "version_number": ver.version_number,
        "ingested_at": ver.ingested_at.isoformat(),
        "raw_file_hash": ver.raw_file_hash,
        "parser_warnings": _json.loads(ver.parser_warnings or "[]"),
...
# ---------------------------------------------------------------------
# Document-wide stale + traceability views
#
# The per-selection / per-node endpoints above require already knowing a
# selection_id or node_id. These two give a whole-document view so a
# reviewer can ask "what, across this entire document, needs re-review"
# without already knowing where to look -- otherwise the staleness feature
# exists but is only reachable if you already suspect where the drift is.
# ---------------------------------------------------------------------

@app.get("/documents/{document_name}/stale")
def get_stale_across_document(document_name: str, db: Session = Depends(get_db)):
    doc = db.query(models.Document).filter_by(name=document_name).first()
    if doc is None:
        raise HTTPException(404, "document not found")

    doc_node_ids = crud.get_all_node_ids_for_document(db, doc.id)
    all_generations = json_store.list_all_generations()
    relevant = [
        g for g in all_generations
        if any(src["node_id"] in doc_node_ids for src in g.get("source_node_revisions", []))
    ]
    with_staleness = [_attach_staleness(db, dict(g)) for g in relevant]
    return [g for g in with_staleness if g["stale"]]


@app.get("/documents/{document_name}/traceability")
def get_traceability(document_name: str, db: Session = Depends(get_db)):
    """node_id -> list of test cases generated from it, with each test
    case's current staleness. Deliberately node-level (not a separate
    'requirement' abstraction) -- see APPROACH.md for why."""
    doc = db.query(models.Document).filter_by(name=document_name).first()
    if doc is None:
        raise HTTPException(404, "document not found")
    ...
    for gen in all_generations:
        node_ids_in_gen = {src["node_id"] for src in gen.get("source_node_revisions", [])}
        relevant_ids = node_ids_in_gen & doc_node_ids
        if not relevant_ids:
            continue
        gen_with_staleness = _attach_staleness(db, dict(gen))
        for node_id in relevant_ids:
            latest_rev = crud.get_latest_revision_for_node(db, node_id)
            entry = {
                "generation_id": gen["_id"],
                "status": gen["status"],
                "stale": gen_with_staleness["stale"],
                "test_case_titles": [tc["title"] for tc in (gen.get("test_cases") or [])],
            }
    ...
    return [
        {
            "node_id": node_id,
            "heading_text": (rev.heading_text if (rev := crud.get_latest_revision_for_node(db, node_id)) else None),
            "test_cases": entries,
        }
